#include "server.hpp"
#include "sql_migrations.hpp" // this header is need for drogon to register plugin
...
Server::Server()
{
    ...
    if (!lead_app_config())
    {
        LOG_ERROR << "Config file not found or incorrect!";
        exit(-1);
    }
}
...
bool Server::lead_app_config()
{
    ...
    if (!postgres_user || !postgres_pass || !postgres_db || !postgres_host || !postgres_port)
    {
